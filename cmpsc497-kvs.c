	      ...
	      unsigned int valsize, unsigned int tagsize, unsigned int padsize )
{
	...
	if ( bufsize > 0 ) {
		unsigned int entrysz = (keysize+valsize+tagsize+padsize); // keysize, valsize, othersize
		bytes_left = bufsize; 
		orig_buf = buf;

		while ( bytes_left >= entrysz ) {  
			err = kvs_auth_set(kvs, buf, buf+keysize, buf+keysize+valsize);  // key at start of buf
			bytes_left -= entrysz;
			buf += entrysz;
		}
		free( orig_buf );
